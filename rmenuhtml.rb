...
class RmenuHtml
  ...
  def call(env)
    begin
      $Rlog.debug("Html Rack") {"call start"}                                   # Logファイル Debug用

      # ブラウザからリクエストデータを受信する
      req      = Rack::Request.new(env)
      req_data = req.params["gamen"]
      $Rlog.debug("Html Rack") {"request data : #{req_data}"}

      url = $Rconfig["base_application"] + "/" + req_data + "/index.html"
      $Rlog.debug("Html Rack") {"HTMLファイルパス: #{url}"}                     # Logファイル Debug用
      ...
      $Rlog.debug("Html Rack") {"call normal end"}
    rescue
      # エラーログを出力する
      $Rlog.debug("Html Rack") {"exception: #{$!}"}                             # Logファイル Debug用
      $Rlog.debug("Html Rack") {"exception: #{$@}"}                             # Logファイル Debug用

      # エラーメッセージを作成する
      content = setErrorMesssage()
      $Rlog.debug("Html Rack") {"call abnormal end"}
    end
