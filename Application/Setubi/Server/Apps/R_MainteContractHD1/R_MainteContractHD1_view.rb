...
class R_MainteContractHD1_view
  ...
  def initialize(response_data, sql_data, request_data)
    begin
      ...
      $Vlog.debug("R_MainteContractHD1_view") {"initialize start"}                    # Logファイル Debug用
      ...
      $Vlog.debug("R_MainteContractHD1_view") {"initialize normal end"}               # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Vlog.error("R_MainteContractHD1_view") {"initialize exception: #{$!}"}         # Logファイル Debug用
      raise
    end
  end

  # 契約保守作業明細データを設定する
  def set契約保守作業明細データOfR_MainteContractHD1OfSetubi()
    $Vlog.debug("R_MainteContractHD1_view") {"set契約保守作業明細データOfR_MainteContractHD1OfSetubi start"}
    ...
    if sqlRecord1["作業区分"]["value"][0] != ""
      $Vlog.debug("R_MainteContractHD1_view") {"set契約保守作業明細データOfR_MainteContractHD1OfSetubi end"}
      return status
    end
