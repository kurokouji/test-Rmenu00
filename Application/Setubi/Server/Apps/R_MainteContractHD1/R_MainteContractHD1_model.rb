...
class R_MainteContractHD1_model
  include SetubiOfBaseNameMainteOfModelMixin

  def initialize(rmenu_dbi, sql_data, request_data)
    begin
      # initialize 開始ログを出力する
      $Mlog.debug("R_MainteContractHD1_model") {"initialize start"}                   # Logファイル Debug用
      ...
      $Mlog.debug("R_MainteContractHD1_model") {"initialize normal end"}              # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Mlog.error("R_MainteContractHD1_model") {"initialize exception: #{$!}"}        # Logファイル Debug用
      raise
    end
  end

  # 契約設備明細の登録データを設定する（登録処理用）
  def set明細登録データOfR_MainteContractHD1OfSetubi()
    $Mlog.debug("R_MainteContractHD1_model") {"set明細登録データOfR_MainteContractHD1OfSetubi start"}
    ...
    $Mlog.debug("R_MainteContractHD1_model") {"set明細登録データOfR_MainteContractHD1OfSetubi end status = #{emptySW}"}
    return emptySW
  end

  # 契約保守諸掛明細・契約保守作業明細・契約設備明細の削除データを設定する
  def set明細削除データOfR_MainteContractHD1OfSetubi(sqlID)
    $Mlog.debug("R_MainteContractHD1_model") {"set明細削除データOfR_MainteContractHD1OfSetubi start"}
    ...
    $Mlog.debug("R_MainteContractHD1_model") {"set明細削除データOfR_MainteContractHD1OfSetubi end status = #{emptySW}"}
    return emptySW
  end

  # 契約設備明細の訂正データを設定する
  def set明細訂正データOfR_MainteContractHD1OfSetubi(sqlID)
    $Mlog.debug("R_MainteContractHD1_model") {"set明細訂正データOfR_MainteContractHD1OfSetubi start"}
    ...
    $Mlog.debug("R_MainteContractHD1_model") {"set明細訂正データOfR_MainteContractHD1OfSetubi end status = #{emptySW}"}
    return emptySW
  end

  # 契約設備明細の登録データを設定する（訂正処理用）
  def set明細訂正登録データOfR_MainteContractHD1OfSetubi(sqlID)
    $Mlog.debug("R_MainteContractHD1_model") {"set明細訂正登録データOfR_MainteContractHD1OfSetubi start"}
    ...
    $Mlog.debug("R_MainteContractHD1_model") {"set明細訂正登録データOfR_MainteContractHD1OfSetubi end status = #{emptySW}"}
    return emptySW
  end
