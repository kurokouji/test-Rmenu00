...
class R_MainteContractSelection_model
  include SetubiOfMasterMainteOfModelMixin

  def initialize(rmenu_dbi, sql_data, request_data)
    begin
      # initialize 開始ログを出力する
      $Mlog.debug("R_MainteContractSelection_model") {"initialize start"}                   # Logファイル Debug用
      ...
      $Mlog.debug("R_MainteContractSelection_model") {"initialize normal end"}              # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Mlog.error("R_MainteContractSelection_model") {"initialize exception: #{$!}"}        # Logファイル Debug用
      raise
    end
  end

  # SQL文の検索条件を変更する（検索データ項目名無し）
  def setSql_R_MainteContractSelectionOfSetubi(idname1, idname2 ,genesql_freesql)
    $Mlog.debug("R_MainteContractSelection_model") {"setSql_R_MainteContractSelectionOfSetubi start"}                  # Logファイル Debug用
    ...
    $Mlog.debug("R_MainteContractSelection_model") {"setSql_R_MainteContractSelectionOfSetubi end"}                    # Logファイル Debug用
    return "OK"
  end
