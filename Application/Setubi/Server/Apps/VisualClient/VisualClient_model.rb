...
class VisualClient_model
  ...
  def initialize(rmenu_dbi, sql_data, request_data)
    begin
      ...
      $Mlog.debug("VisualClient_model") {"initialize normal end"}              # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Mlog.error("VisualClient_model") {"initialize exception: #{$!}"}        # Logファイル Debug用
      raise
    end
  end
	
  # SQL文の検索条件を変更する（検索データ項目名無し）
  def setSql_VisualClientOfSetubi(idname1, idname2 ,genesql_freesql)
    $Mlog.debug("VisualClient_model") {"setSql_VisualClientOfSetubi start"}                  # Logファイル Debug用
    ...
    $Mlog.debug("VisualClient_model") {"setSql_VisualClientOfSetubi end"}                    # Logファイル Debug用
    return "OK"
  end
