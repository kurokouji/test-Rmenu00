...
class VisualJson_model
  ...
  def initialize(rmenu_dbi, sql_data, request_data)
    begin
      ...
      $Mlog.debug("VisualJson_model") {"initialize normal end"}              # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Mlog.error("VisualJson_model") {"initialize exception: #{$!}"}        # Logファイル Debug用
      raise
    end
  end
	
  # SQL文の検索条件を変更する（検索データ項目名無し）
  def setSql_VisualJsonOfSetubi(idname1, idname2 ,genesql_freesql)
    $Mlog.debug("VisualJson_model") {"setSql_VisualJsonOfSetubi start"}                  # Logファイル Debug用
    ...
    $Mlog.debug("VisualJson_model") {"setSql_VisualJsonOfSetubi end"}                    # Logファイル Debug用
    return "OK"
  end
