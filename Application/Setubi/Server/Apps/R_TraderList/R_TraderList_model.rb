...
class R_TraderList_model
  ...
  def initialize(rmenu_dbi, sql_data, request_data)
    begin
      ...
      $Mlog.debug("R_TraderList_model") {"initialize normal end"}              # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Mlog.error("R_TraderList_model") {"initialize exception: #{$!}"}        # Logファイル Debug用
      raise
    end
  end

  # SQL文の検索条件を変更する（検索データ項目名無し）
  def setSql_R_TraderListOfSetubi(idname1, idname2 ,genesql_freesql)
    $Mlog.debug("R_TraderList_model") {"setSql_R_TraderListOfSetubi start"}                  # Logファイル Debug用
    ...
    $Mlog.debug("R_TraderList_model") {"setSql_R_TraderListOfSetubi end"}                    # Logファイル Debug用
    return "OK"
  end
