...
class R_TraderList_view
  ...
  def initialize(response_data, sql_data, request_data)
    begin
      ...
      $Vlog.debug("R_TraderList_view") {"initialize start"}                    # Logファイル Debug用
      ...
      $Vlog.debug("R_TraderList_view") {"initialize normal end"}               # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Vlog.error("R_TraderList_view") {"initialize exception: #{$!}"}         # Logファイル Debug用
      raise
    end
  end
