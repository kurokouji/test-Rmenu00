...
module SetubiOfBatchStartUpOfModelMixin
  ...
  def batch_startup(projectName, programName, mode, startupProgramName)
    $Mlog.debug("SetubiOfBatchStartUpOfModelMixin") {"batch_startup start"}
    ...
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_startup end"}
    return "OK"
  end
  ...
  def batch_paramkeyget(projectName, programName, mode)
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_paramkeyget start"}
    ...
    str_sql     += " GROUP BY"
    str_sql     += " プロジェクト名, プログラム名, モード"
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"sql1 = #{str_sql}"}
    ...
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"sql2 = #{str_sql}"}
    valueArray   = @rmenu_db["default"].select_namevalue(str_sql)
    
    if valueArray.empty?
      $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"バッチ起動パラメータがありません。"}
      $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_paramkeyget abnormal end"}
      return "ERROR"
    end
    

    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"keyRecord   = #{valueArray[0]["バッチキーデータ"]}"}
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"paramRecord = #{valueArray[0]["バッチパラメータ"]}"}
    ...
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_paramkeyget end"}
    return "OK"
  end
  
  # バッチ起動パラメータ　登録
  def batch_startup_insert(projectName, programName, mode, startNo, keyRecord, paramRecord)
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_startup_insert start"}
    ...
    str_sql     += ")"
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"sql = #{str_sql}"}
    @rmenu_db["default"].insert(str_sql)
        
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_startup_insert end"}
    return "OK"
  end
  
  # バッチ起動パラメータ　削除
  def batch_startup_delete(projectName, programName, mode, startNo)
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_startup_delete start"}
    ...
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_startup_delete end"}
    return "OK"
  end
  
  # Rubyプログラム　起動
  def batch_startup_ruby(projectName, startupProgramName)
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_startup_ruby start"}

    command = "ruby " + $Rconfig['base_application'] + "/" + projectName + "/Server/BachProcess/" + startupProgramName + ".rb"
    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"command = #{command}"}
    Open3.popen3(command)

    $Mlog.debug("SetubiOfBaseNameMainteOfModelMixin") {"batch_startup_ruby end"}
    return "OK"
  end
