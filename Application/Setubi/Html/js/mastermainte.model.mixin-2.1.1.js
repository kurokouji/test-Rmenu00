...
(function($, $R){
  ...
  App.MasterMainteModelMixin = {
   ...
   ,onServerDialogAfter: function() {
    ...
    // -----------------------------------
   ,set表示順: function() {
      $R.log("Model set表示順 : start");

      var dataSet       = this.dataset.getData();
      var dataSetInfo   = this.appspec.getJSONChunkByIdAtRecords(dataSet, "header");
			var dataSetRecord = dataSetInfo["record"];
			var targetName    = "";
      ...
      var fromNumber = sessionStorage.loadItem("fromNumber");
      var toNumber   = sessionStorage.loadItem("toNumber");
			
			var value      = 0;
			if (fromNumber) {
			  if (toNumber) {
			    value = new BigNumber(fromNumber).plus(toNumber).div(2).round(0,1).toPrecision();
			  }
			  else {
			    value = new BigNumber(fromNumber).plus(2).toPrecision();
			  }
			}
			else {
				value = 10;
			...
			dataSetRecord[targetName]["value"][0] = value;
			var arg = {項目名: targetName, 表示順: value};

      $R.log("Model set表示順 : end");
			return arg; 
    }
