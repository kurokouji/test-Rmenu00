(function($, $R){
  // 名前空間を設定する
  var App = $R.Application.XX_EquipmentType;
  ...
  Controller.include({
    ...
    // ---------------------------------------
     on設備タイプ別作業: function(event) {
      ...
      // 次画面の処理モードを設定する
      this.model.saveSessionStorageOfNextMode("XX_EquipmentTypeWork", "execute");

      // 次画面に画面遷移する
      this.model.on次画面表示("XX_EquipmentTypeWork", "execute");
