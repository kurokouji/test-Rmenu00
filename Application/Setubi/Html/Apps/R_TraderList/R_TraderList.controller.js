(function($, $R){
  // 名前空間を設定する
  var App = $R.Application.R_TraderList;
  ...
  Controller.include({
    ...
    // ---------------------------------------
     on業者別扱い設備タイプ: function(event) {
      $R.log("Controller on業者別扱い設備タイプ : start");

      // 次画面の処理モードを設定する
      this.model.saveSessionStorageOfNextMode("R_TraderEquipmentType", "execute");

      // 次画面に画面遷移する
      this.model.on次画面表示("R_TraderEquipmentType", "execute");

      $R.log("Controller on業者別扱い設備タイプ : end");
    }

    ,on保守契約一覧: function(event) {
      $R.log("Controller on保守契約一覧 : start");
      
      // 保守契約一覧画面への引継ぎデータ（顧客名称）をセット
