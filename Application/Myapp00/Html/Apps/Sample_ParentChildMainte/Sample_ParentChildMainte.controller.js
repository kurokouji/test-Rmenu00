(function($, $R){
  // 名前空間を設定する
  var App = $R.Application.Sample_ParentChildMainte;
  ...
  Controller.include({
   ...
   ,checkサンプル_明細数量: function(arg) {
      $R.log("Controller checkサンプル_明細数量 : start");

      var arg1 = this.model.checkサンプル_明細数量(arg);
      var arg2 = this.model.deriveサンプル_明細金額(arg1);
      this.view.deriveサンプル_明細金額(arg2);

      $R.log("Controller checkサンプル_明細数量 : end");
      return arg;
    }
   ...
   ,checkサンプル_明細単価: function(arg) {
      $R.log("Controller checkサンプル_明細単価 : start");

      var arg1 = this.model.checkサンプル_明細単価(arg);
      var arg2 = this.model.deriveサンプル_明細金額(arg1);
      this.view.deriveサンプル_明細金額(arg2);
      
      $R.log("Controller checkサンプル_明細単価 : end");
      return arg;
    }
