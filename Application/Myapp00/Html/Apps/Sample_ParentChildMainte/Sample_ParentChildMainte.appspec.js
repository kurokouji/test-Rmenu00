(function($, $R){
  ...
  AppSpec.fn.init = function(name) {
    ...
    this.name       = name;
    this.nextname   = "";
    this.beforename = "Sample_ParentChildList";
  ...
  AppSpec.include({
    ...
    // -----------------------------------------------------------------------------------------------
    urlInfo: [
      {app:  "Myapp00/Html/Apps/Sample_ParentChildMainte/"}
     ,{json: "Myapp00/Json/Apps/Sample_ParentChildMainte/"}
    ]
    // --------------------------
    // JSONファイル情報を定義する
    // --------------------------
   ...
   ,pubsubCheckEvent: [
    // ここから追加処理
      ["checkサンプル_明細数量", "controller", "checkサンプル_明細数量"]
     ,["checkサンプル_明細単価", "controller", "checkサンプル_明細単価"]
    ]
