(function($, $R){
  // 名前空間を設定する
  var App = $R.Application.T_Sample_ParentChildMainte = {};
  ...
  AppSpec.fn.init = function(name) {
    ...
    this.name       = name;
    this.nextname   = "";
    this.beforename = "T_Sample_ParentChildList";
  ...
  AppSpec.include({
    ...
    // -----------------------------------------------------------------------------------------------
    urlInfo: [
      {app:  "Myapp00/Html/Apps/T_Sample_ParentChildMainte/"}
     ,{json: "Myapp00/Json/Apps/T_Sample_ParentChildMainte/"}
    ]
    // --------------------------
    // JSONファイル情報を定義する
    // --------------------------
   ...
   ,selectStorageRequestData: [
      {
         requestname: "T_Sample_Selection"
        ,datasetid:   ""
        ,dataname:   []
        ,value:      []
      }
   ...
   ,selectStorageResponseData: [
      {
         responsename: "T_Sample_ParentChildMainte"
        ,datasetid:    "header"
