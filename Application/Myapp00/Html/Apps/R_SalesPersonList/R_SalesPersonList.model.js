(function($, $R){
  // 名前空間を設定する
  var App = $R.Application.R_SalesPersonList;
  ...
  Model.include({
    ...
    // ---------------------------------------
     on保守契約一覧前処理: function(event) {
      ...
      if (headerRecord["検索営業担当者名"]["value"][0] != "") {
        return false;
      }

      // 保守契約一覧画面への引継ぎデータ（検索営業担当者名）をセット
      sessionStorage.setIdName(this.appspec.sysname + "." + this.appspec.name);
      var clickRow      = sessionStorage.loadItem("クリック行");

      w_検索営業担当者名 = detailRecord["営業担当者名"]["value"][clickRow];
      headerRecord["検索営業担当者名"]["value"][0] = w_検索営業担当者名;
    ...
    ,on保守契約一覧後処理: function(event) {
      ...
      // セッションストレージ　検索営業担当者名　クリア
      sessionStorage.setIdName(this.appspec.sysname + "." + this.appspec.name);
      sessionStorage.saveItem("検索営業担当者名", "");
