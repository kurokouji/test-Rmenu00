...
class Sample_ParentMainte_model
  ...
  def set明細行番号OfSample_ParentMainteOfMyapp00()
    $Mlog.debug("Sample_ParentMainte_model") {"set明細行番号OfSample_ParentMainteOfMyapp00 start"}
    ...
    $Mlog.debug("Sample_ParentMainte_model") {"set明細行番号OfSample_ParentMainteOfMyapp00 end status = #{emptySW}"}
    return emptySW
  end
