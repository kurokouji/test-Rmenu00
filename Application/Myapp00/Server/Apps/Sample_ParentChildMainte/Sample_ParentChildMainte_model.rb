...
class Sample_ParentChildMainte_model
  ...
  def set明細行番号OfSample_ParentChildMainteOfMyapp00()
    $Mlog.debug("Sample_ParentChildMainte_model") {"set明細行番号OfSample_ParentChildMainteOfMyapp00 start"}
    ...
    $Mlog.debug("Sample_ParentChildMainte_model") {"set明細行番号OfSample_ParentChildMainteOfMyapp00 end status = #{emptySW}"}
    return emptySW
  end
