...
class R_CustomersSelection_view
  ...
  def initialize(response_data, sql_data, request_data)
    begin
      ...
      $Vlog.debug("R_CustomersSelection_view") {"initialize start"}                    # Logファイル Debug用
      ...
      $Vlog.debug("R_CustomersSelection_view") {"initialize normal end"}               # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Vlog.error("R_CustomersSelection_view") {"initialize exception: #{$!}"}         # Logファイル Debug用
      raise
    end
  end
