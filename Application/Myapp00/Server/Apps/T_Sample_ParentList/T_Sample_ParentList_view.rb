...
class T_Sample_ParentList_view
  ...
  def initialize(response_data, sql_data, request_data)
    begin
      ...
      $Vlog.debug("T_Sample_ParentList_view") {"initialize start"}                    # Logファイル Debug用
      ...
      $Vlog.debug("T_Sample_ParentList_view") {"initialize normal end"}               # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Vlog.error("T_Sample_ParentList_view") {"initialize exception: #{$!}"}         # Logファイル Debug用
      raise
    end
  end
