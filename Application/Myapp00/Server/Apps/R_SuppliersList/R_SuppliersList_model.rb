...
class R_SuppliersList_model
  include Myapp00OfMasterMainteOfModelMixin

  def initialize(rmenu_dbi, sql_data, request_data)
    begin
      # initialize 開始ログを出力する
      $Mlog.debug("R_SuppliersList_model") {"initialize start"}                   # Logファイル Debug用
      ...
      $Mlog.debug("R_SuppliersList_model") {"initialize normal end"}              # Logファイル Debug用
    rescue Exception
      # エラーログを出力する
      $Mlog.error("R_SuppliersList_model") {"initialize exception: #{$!}"}        # Logファイル Debug用
      raise
    end
  end

  # SQL文の検索条件を変更する（検索データ項目名無し）
  def setSql_R_SuppliersListOfMyapp00(idname1, idname2 ,genesql_freesql)
    $Mlog.debug("R_SuppliersList_model") {"setSql_R_SuppliersListOfMyapp00 start"}                  # Logファイル Debug用
    ...
    $Mlog.debug("R_SuppliersList_model") {"setSql_R_SuppliersListOfMyapp00 end"}                    # Logファイル Debug用
    return "OK"
  end
