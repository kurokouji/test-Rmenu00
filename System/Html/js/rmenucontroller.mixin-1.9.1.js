(function($, $R){
  ...
  App.ControllerMixin = {
   ...
   ,onControllerBlur: function(event) {
    ...
    // テーブルにクリックイベントを設置する
    // -------------------------------------------------------
   ,setClickEventOfTable: function(tableID, callBack) {
      $R.log("ControllerMixin setClickEventOfTable : start");

      var self = this;
      var rows = $(tableID)[0].rows;
      var fnc  = "self." + callBack + "(i)";
      jQuery.each(rows, function(i) {
        // コールバック関数を設置する
        rows[i].onclick = function () {eval(fnc);}
      });

      $R.log("ControllerMixin setClickEventOfTable : end");
    }
    // データセットとフォームの値をクリアする
