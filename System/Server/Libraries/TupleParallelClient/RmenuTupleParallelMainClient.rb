...
class RmenuTupleParallelMainClient
  ...
  def initialize(log_file_name, tuple_parallel_server)
    ...
    begin
      ...
      @tupleparallelserver           = tuple_parallel_server                              # タプルスペース
  ...
  def call()
    begin
      ...
      # ループ開始
      loop do
			  # タプル並列サーバのcallメソッドを呼び出す
        tuple                 = @tupleparallelserver.call("tapleclient", "")
        ...
        rmenuTupleParallelMainController  = RmenuTupleParallelMainController.new(@db_queue, rmenu_db)
        ...
        sql_data = rmenuTupleParallelMainController.call(rmenuParallelControll, parallel_controll_id, parallel_division_id)
        ...
        if nextStatus
          status = @tupleparallelserver.call("tapleserver", sql_data)
          $Tlog.debug("TupleMainClient") {"NEXTジョブ　起動要求ステータス：#{status}"}
			  end
